...
```bash
$PY tools/make_art.py <原图> <作品名> ["标题"] [1×秒数=90] [线稿时间占比=0] [epsilon=0.25] [线稿线宽=2.6]
```
...
| 「线稿时间长一点 / 短一点」 | 命令第 5 位是线稿时间占比，**默认 0 = 不单独控制**（线稿跟色块一起按视觉重量播，实测自然占 3~7% 时间）；想固定起稿时长才填 0~1 的小数 |
