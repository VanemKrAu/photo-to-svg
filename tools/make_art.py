...
用法：
  ...
  --desc="…"            图片描述：写进回放页顶栏「描述」，收起显示一行、点击展开编辑。
                        agent 干活时先看图写详细描述（主体 / 动作 / 姿态 / 细节 / 场景 /
                        光线 / 色调；如实写、不做道德化回避），再传入；
...
# SVG 的底板 rect 与回放页的 canvas 背景都取自这一个值，两处必须同源 ——
# （2026-09-14 起「与底板相近整簇跳过」的省笔已取消，底板只当纸用。）
PAPER = "#f0f0f0"
...
def pick_background(photo):
    ...
    底板现在只当「纸」用：2026-09-14 起「与底板色差 ≤3 的色块整簇跳过不画」的
    省笔已取消 —— 所有色块都实打实画出来。SVG 的底板 rect 与回放页的 canvas
    背景都取自这里，两处永远同源（历史来由见踩坑记录 #20）。

    历史上这里按画面自动取色（高频色 >12% 就用它，否则按明暗给 #f0f0f0/#101010），
    深色图因此拿到深色底板。改成固定纸白后：
      * 浅色图：与旧行为一致（本来就是 #f0f0f0）
      * 深色图：深色区也会一笔笔画在白纸上 —— 要的就是「纸上作画」的观感；
        代价是笔数与体积上升，这是白底必然的账
